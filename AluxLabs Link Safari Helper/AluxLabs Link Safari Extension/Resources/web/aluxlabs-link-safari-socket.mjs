...
const messageListener = function (event) {
    const message = event.data['from-aluxlabs-link'];
    if (!message) return;
...
/**
 * AluxLabsLinkSafariSocket class: represents a AluxLabs Link session socket using the Safari extension as transport.
 */
class AluxLabsLinkSafariSocket {
    ...
    /**
     * Construct a new AluxLabs Link session socket.
    ...
    /**
     * Open communication with AluxLabs Link.
    ...
    /**
     * Send a message to AluxLabs Link
    ...
    /**
     * @param {function} callback - the function to call when a message is received from AluxLabs Link
     */
    ...
    _sendNotify (method, params = {}) {
        this._toAluxLabsLink(method, params, false);
    }

    _sendRequest (method, params = {}) {
        return this._toAluxLabsLink(method, params, true);
    }

    /**
     * Send a notification or request to AluxLabs Link.
     ...
     */
    _toAluxLabsLink (method, params, expectResponse) {
        const message = {
            jsonrpc: '2.0',
            session: this._id,
            method
        ...
        self.postMessage({'to-aluxlabs-link': message}, self.origin);
        return responsePromise;
    }
}

export { AluxLabsLinkSafariSocket };
