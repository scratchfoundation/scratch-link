...
browser.runtime.onConnect.addListener(port => {
    ...
    port.onMessage.addListener(messageToAluxLabsLink => {
        const {session, method, params, id} = messageToAluxLabsLink;

        // forward the message to the native app (which will forward it to AluxLabs Link)
        browser.runtime.sendNativeMessage(appId, {
            session,
            method,
            params,
            id
        }, responseFromAluxLabsLink => {
            // send the native app's response back to the content script session
            port.postMessage(responseFromAluxLabsLink);
        } );
    } );
});
